...
node('master_pt') {
  ...
  stage ('Deploy '){
	...
	sshPublisher alwaysPublishFromMaster: true, publishers: [sshPublisherDesc(configName: 'BHMC_deploy', sshCredentials: [encryptedPassphrase: '{AQAAABAAAAAQLitmD9oMOPIon0LQHqyuaZiJ9ZU3SOyUFEzKYl88FCc=}', key: '', keyPath: '', username: 'root'], transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: 'echo \'Hello Deploy Success In BHMC\' >> echo.out', execTimeout: 120000, flatten: false, makeEmptyDirs: true, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '/jenkins_deploy_test/', remoteDirectorySDF: true, removePrefix: '/root/nems2/', sourceFiles: '/target/*.jar')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)]
	sshPublisher alwaysPublishFromMaster: true, publishers: [sshPublisherDesc(configName: 'DYK_deploy', sshCredentials: [encryptedPassphrase: '{AQAAABAAAAAQ17RZCQuR3a0A1lxVM0z8VdmdwQhUmey2IYmXBaUcTSQ=}', key: '', keyPath: '', username: 'root'], transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: 'echo \'Hello Deploy Success In DYK\' >> echo.out', execTimeout: 120000, flatten: false, makeEmptyDirs: true, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '/jenkins_deploy_test/', remoteDirectorySDF: true, removePrefix: '/root/nems2/', sourceFiles: '/target/*.jar')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)]
  }
